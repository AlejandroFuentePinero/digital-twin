...
- `data/readmes/` is **tool-only content** — 29 distilled project / paper docs the GENERIC, GAP, and TECHNICAL branches can fetch via `fetch_project_readme` (per `branches.REGISTRY`; widened from TECHNICAL-only in Session 56 commit `219bfb8`). Outside `data/knowledge_base/`, never ingested. Registry at `data/readmes/registry.json` (29 keys; ToolRegistry hard-fails at startup if any referenced file is missing).
