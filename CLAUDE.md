...
- One enriched interaction log (schema v3 — `is_canary`, `run_id`, `replicate_index` added in #39) replaces the three-file plan in the original PLAN.md. Schema at `interaction_log.InteractionRecord`. Live and canary records share `data/logs/interactions.jsonl`; the `is_canary` flag is the only discriminator. `DashboardModel` filters canary records out by default so live tabs are unaffected.
