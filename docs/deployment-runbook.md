...
> **Last deploy:** 2026-08-19 (content change, batched). Trigger: MTG Deck Optimisation Engine added to the KB, shipped together with the two course additions that had accumulated undeployed since Aug 5 / Aug 7. KB changed → re-ingest ran (107 chunks) before upload. Canary `+N` point `run-20260819-063110-c59dcd`: 14 flags (9 major) vs the frozen baseline, up from 6 (Aug), 9 (Jul), 12 (May) — first increase in the trajectory, and triage found zero content regressions. 6 of the 9 major flags are two questions moving *toward* the corpus contract (phishing request now deflected rather than gap-acknowledged; ML-experience question now answers with substance because the Officeworks content landed), and 3 are keyword-substring artefacts where the fact is stated correctly in different words. Space stage `RUNNING` on `cpu-basic`, 56 files / 28.1 MB, targeted DB upload a no-op as expected, browser smoke-test passed on the MTG probe with all three project links correct. See `DECISIONS.md` Session 66 for the per-flag triage. Uncheck the gates above before the next deploy and re-run.
>
> **Previous deploy:** 2026-08-03 (content change). Trigger: 7PH Graph added to the KB + a 29th `data/readmes/` entry. KB changed → re-ingest ran (106 chunks) before upload. Canary `+N` point `run-20260803-012541-e0d751`: 6 major flags vs the frozen baseline, down from 9 (Jul) and 12 (May); triaged as 2 improvements (CUDA question now matches its `gap_acknowledged` expectation), 1 benign `must_not_appear` substring collision, 1 pre-existing, 2 mild keyword-coverage drops. Space stage `RUNNING` on `cpu-basic` after ~135 s, HTTP 200, registry 29 keys + `7ph_graph.md` + the new KB section confirmed on the Space's filesystem, browser smoke-test passed on the knowledge-graph/open-source probe. Uncheck the gates above before the next deploy and re-run.
>
