...
**Last updated:** 2026-05-01 (Session 18)
**Current phase:** Phase 2 progressing fast. Issues `#13` (foundation), `#14` (Career Timeline), and `#15` (real classifier + GAP branch) all closed. The routed pipeline is live with two branches (GENERIC + GAP), real `gpt-4.1-nano` classifier, multi-label routing, 5-layer active-learning defense for in-progress curriculum keywords, hand-editable runtime pipeline diagram in MAP.md, sequential smoke-test runbook in `HUMAN_EVAL_QUESTIONS.md`, and end-of-project verification list in `RELEASE_CHECKLIST.md`. **Issue `#21` (live smoke-test) is queued next** and effectively gates further branch work (`#17`, `#18`, `#19`) until real classifier accuracy and active-learning defense are validated against live OpenAI traffic. Phase 1 sub-task remaining: rewrite `data/knowledge_base/positioning.md` to remove transfer-principle prose now in `profile.md`. Test count 134/134; KB at 109 chunks. New tooling: hand-edited [`docs/pipeline_diagram.mmd`](./pipeline_diagram.mmd) (renders into MAP.md alongside the auto-generated module graph); [`docs/RELEASE_CHECKLIST.md`](./RELEASE_CHECKLIST.md); [`docs/HUMAN_EVAL_QUESTIONS.md`](./HUMAN_EVAL_QUESTIONS.md) restructured as a sequential smoke-test runbook with stable question IDs.
