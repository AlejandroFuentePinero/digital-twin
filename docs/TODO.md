...
**Current phase:** Phase 2 — four branches wired (GENERIC + GAP + LOGISTICAL + BEHAVIOURAL). Issues closed: `#13`, `#14`, `#15` (foundation + GAP), `#21` + `#26` (R1 + R2 smoke-tests), `#22` + `#24` + `#25` (polish), `#19` (LOGISTICAL branch), `#17` (BEHAVIOURAL branch + deflection rule + Story 8). The routed pipeline is live with four branches; real `gpt-4.1-nano` classifier; multi-label routing; 5-layer active-learning defense; deterministic guardrail short-circuit on the canonical refusal phrase; soft conciseness nudge across all four branches' `branch_rules`; honest deflection on behavioural off-list questions. KB date-stable past 2026-05-13. Standing concerns logged in [`docs/LIMITATIONS.md`](./LIMITATIONS.md) (5 observed + 7 predicted entries with explicit trip-wires). **Next: issue `#18` (TECHNICAL branch + tool loop)** — final branch in the registry; body must absorb the `LIMITATIONS.md::O2` watch-item (TECHNICAL classifier over-firing on tool-name probes). Test count 146/146; KB at 109 chunks. Phase 1 sub-task remaining: rewrite `data/knowledge_base/positioning.md`.
