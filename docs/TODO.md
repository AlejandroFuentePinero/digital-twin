...
**Current phase:** Phase 2 progressing fast. Issues `#13` (foundation), `#14` (Career Timeline), `#15` (real classifier + GAP branch), and `#21` (live smoke-test validation) all closed. The routed pipeline is live with two branches (GENERIC + GAP), real `gpt-4.1-nano` classifier, multi-label routing, 5-layer active-learning defense for in-progress curriculum keywords. **Smoke-test #21 validated #15 empirically** — 20 pass / 3 partial / 3 fail across 26 questions; active-learning defense 8/8 on curriculum-keyword probes; filter-fallback masking misroutes silently. Three small polish issues opened: `#24` (KB stale-date hardening — highest priority, KB-first per user), `#22` (guardrail-accepts-gap-phrase), `#25` (soft conciseness rule). One closed as not-planned (`#23`). Effective gating on `#17`/`#18`/`#19` lifted. Phase 1 sub-task remaining: rewrite `data/knowledge_base/positioning.md`. Test count 134/134; KB at 109 chunks (will re-ingest with #24). Tooling: hand-edited [`docs/pipeline_diagram.mmd`](./pipeline_diagram.mmd) (renders into MAP.md alongside the auto-generated module graph); [`docs/RELEASE_CHECKLIST.md`](./RELEASE_CHECKLIST.md); [`docs/HUMAN_EVAL_QUESTIONS.md`](./HUMAN_EVAL_QUESTIONS.md) sequential smoke-test runbook with stable question IDs.
