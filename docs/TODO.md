...
**Last updated:** 2026-05-03 (Session 20)
**Current phase:** Phase 2 progressing fast. Issues `#13` (foundation), `#14` (Career Timeline), `#15` (real classifier + GAP branch), `#21` (live smoke-test validation), `#22` (guardrail accepts gap phrase), `#24` (KB stale-date hardening), and `#25` (soft conciseness rule) all closed. The routed pipeline is live with two branches (GENERIC + GAP), real `gpt-4.1-nano` classifier, multi-label routing, 5-layer active-learning defense, deterministic guardrail short-circuit on the canonical refusal phrase, and a soft conciseness nudge in both branches' `branch_rules`. KB date-stable past 2026-05-13: Officeworks presented as currently held, ecology timeframe 2014–2026, explicit availability line. **Issue `#26` (second-round human eval) is queued next** — regression + intended-effect check after the polish round. Test count 134 → 138; KB at 109 chunks. Phase 1 sub-task remaining: rewrite `data/knowledge_base/positioning.md`. Tooling: hand-edited [`docs/pipeline_diagram.mmd`](./pipeline_diagram.mmd); [`docs/RELEASE_CHECKLIST.md`](./RELEASE_CHECKLIST.md); [`docs/HUMAN_EVAL_QUESTIONS.md`](./HUMAN_EVAL_QUESTIONS.md) sequential smoke-test runbook.
