...
**Last updated:** 2026-05-03 (Session 22)
**Current phase:** Phase 2 — three branches wired (GENERIC + GAP + LOGISTICAL). Issues closed: `#13`, `#14`, `#15` (foundation + GAP), `#21` + `#26` (R1 + R2 smoke-tests), `#22` + `#24` + `#25` (polish), `#19` (LOGISTICAL branch). The routed pipeline is live with three branches; real `gpt-4.1-nano` classifier; multi-label routing; 5-layer active-learning defense; deterministic guardrail short-circuit on the canonical refusal phrase; soft conciseness nudge across all three branches' `branch_rules`. KB date-stable past 2026-05-13. Standing concerns logged in [`docs/LIMITATIONS.md`](./LIMITATIONS.md) (5 observed + 6 predicted entries with explicit trip-wires). **Next: issue `#17` (BEHAVIOURAL + deflection rule)** — additive shape just validated by #19; #17 layers in a novel branch rule on top. Test count 141/141; KB at 109 chunks. Phase 1 sub-task remaining: rewrite `data/knowledge_base/positioning.md`.
