...
2. Aggregate call rate over time (Sentinel's `technical_tool_call_rate` + Trend Explorer) shows TECHNICAL turns calling the tool at a rate that doesn't match the question shape (e.g., consistently 0% when realistic question mix should be 40-60%).
...
**What would help observe this cleanly:** a Sentinel panel showing `branch == TECHNICAL` turn count vs `len(tool_calls) > 0` count, broken down by question shape (project-named vs general). The aggregate `technical_tool_call_rate` exists today; the question-shape breakdown is not yet built (would be a custom Sentinel surface — `#34`'s `FlagDetector` ships `gap_rate_jump` / `new_cluster` / `repeat_failure` only, not question-shape disaggregation).

**Post-`#45` framing:** the canary surface no longer asserts mechanism (`requires_tool` was the contract for the now-removed `tool_uptake_on_warranted(corpus)` metric). PRD `#41` slice 4 replaced the mechanism contract with an outcome contract — `outcome_accuracy` / `keyword_coverage` / `red_flag_rate` measure whether the answer was substantively correct rather than whether the tool fired. The live `technical_tool_call_rate` keeps its descriptive role on the live tab (PRD `#41` slice 3) — same noisy denominator by design (denominator includes every TECHNICAL turn, including meta-questions and follow-ups whose context is already in scope).
...
**Bulk-strip exception (PRD `#41` slice 4 / `#45`):** the pre-`#45` 226 canary records (76 orphan + 150 contaminated baseline) were stripped wholesale rather than left as orphans because the underlying *contract* changed (mechanism-shaped → outcome-shaped); the records were unreadable under the new schema. Operator runbook documented in `docs/audits/slice-4-canary-recalibration.md` § 9. Bulk-strip is **not** the default recovery; future credit / rate-limit failures still follow the orphan-leave procedure above.
